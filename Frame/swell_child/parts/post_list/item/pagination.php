...
<?php
	...
	if ( 1 === $the_pages ) :
		...
		// カスタマイズ：Previousボタン（一つ戻る）
		if ($the_paged > 1) :
			echo '<a href="' . esc_url( get_pagenum_link( $the_paged - 1 ) ) . '" class="my-prev-next my-pagi-prev"><i class="fas fa-angle-left"></i>PREV</a>';
		endif;
		...
		for ( $i = 1; $i <= $the_pages; $i++ ) :
			...
			elseif ( $apart <= $range ) :
				echo '<a href="' . esc_url( get_pagenum_link( $i ) ) . '" class="page-numbers' . $add_class . '" data-apart="' . $apart . '">' . $i . '</a>';
			endif;
		endfor;
		...
		if ( $the_paged + $range < $the_pages ) :
			echo '<span class="c-pagination__dot">...</span>';
			echo '<a href="' . esc_url( get_pagenum_link( $the_pages ) ) . '" class="page-numbers -to-last">' . $the_pages . '</a>';
		endif;

		// カスタマイズ：NEXTボタン（次のページへ）
		if ($the_paged < $the_pages) :
			echo '<a href="' . esc_url( get_pagenum_link( $the_paged + 1 ) ) . '" class="my-prev-next my-pagi-next">NEXT<i class="fas fa-angle-right"></i></a>';
		endif;
